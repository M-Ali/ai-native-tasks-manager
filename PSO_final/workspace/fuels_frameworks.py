"""
PSO Fuels — Strategic Framework Analysis
Mirrors workspace/frameworks.py (Lubricants), adapted to Fuels categories
(Diesel, Petrol, Other Fuels, LPG).
...
Output:    reports/PSO_Fuels_Framework_Analysis_<period>.pptx
"""
import sys, io, os
...
CAT_COLORS = {'Diesel': H_NAVY, 'Petrol': H_GREEN, 'Other Fuels': H_ORANGE, 'LPG': H_RED}
...
def _hdr(sl, title, sub=''):
    ...
    if sub:
        _txt(sl, 0.25, 0.40, 9.5, 0.18, sub, size=7.5, color=RGBColor(0xBB, 0xCC, 0xFF))
...
def _sidebar_card(sl, rx, by, cat_name, quad_text, detail, col_rgb):
    ...
    _txt(sl, rx+0.12, by+0.06, 2.2, 0.22, cat_name, size=8, bold=True, color=C_DNAVY)
    _txt(sl, rx+0.12, by+0.30, 2.5, 0.20, quad_text, size=8, bold=True, color=col_rgb)
    _txt(sl, rx+0.12, by+0.52, 3.10, 0.32, detail, size=7, color=RGBColor(0x44, 0x44, 0x44), wrap=True)
...
MAIN = [c for c in ['Diesel', 'Petrol', 'Other Fuels', 'LPG'] if (fuels['FuelSegment'] == c).any()]
...
total_stns = fuels['Customer Number'].nunique()
cat['stn_pct'] = cat['stns'] / total_stns * 100
cat['kl_per_stn'] = cat['vol_kl'] / cat['stns']

# ── ① BCG Growth-Share Matrix ─────────────────────────────────────────────────
BCG_SHARE_THRESH = cat['share'].median()
BCG_GROWTH_THRESH = cat['growth'].median()
...
BCG_RGB = {'Star': C_GOLD, 'Cash Cow': C_GREEN, 'Question Mark': C_NAVY, 'Dog': C_RED}
BCG_HEX = {'Star': H_GOLD, 'Cash Cow': H_GREEN, 'Question Mark': H_NAVY, 'Dog': H_RED}
...
VM_RGB = {'Champion': C_GOLD, 'Volume Driver': C_NAVY, 'Premium Star': C_GREEN, 'Restructure': C_RED}
...
DE_RGB = {'Optimise': C_GREEN, 'Push Productivity': C_ORANGE, 'Expand Reach': C_NAVY, 'Rethink': C_RED}
DE_HINT = {
...
PLC_HEX = {'Growth': H_GREEN, 'Late Growth': '#6EC048', 'Maturity': H_ORANGE,
           'Late Maturity': '#E4A030', 'Decline': H_RED}
...
for c, r in cat.iterrows():
    print(f"{c:<16}  {r.share:>7.1f}  {r.growth:>+8.1f}  {r.bcg:>14}  {r.vm:>14}  {r.de:>16}  {r.plc:>12}")
...
def _bubble(ax, x_col, y_col, size_col, size_max, size_scale=2500):
    for c_name, row in cat.iterrows():
        ...
        ax.scatter(row[x_col], row[y_col], s=sz, color=col, alpha=0.82, edgecolors='white', linewidth=2, zorder=5)
        ax.annotate(c_name.replace(' ', '\n'), (row[x_col], row[y_col]),
                    textcoords='offset points', xytext=(10, 4), fontsize=7.5, fontweight='bold', color=col)

def _quad_bg(ax, thresh_x, thresh_y, xmin, xmax, ymin, ymax):
    yn = (thresh_y - ymin) / (ymax - ymin)
    xn = (thresh_x - xmin) / (xmax - xmin)
    ax.axvspan(thresh_x, xmax, ymin=yn, ymax=1, alpha=0.07, color=H_GOLD)
    ax.axvspan(thresh_x, xmax, ymin=0, ymax=yn, alpha=0.07, color=H_GREEN)
    ax.axvspan(xmin, thresh_x, ymin=yn, ymax=1, alpha=0.07, color=H_NAVY)
    ax.axvspan(xmin, thresh_x, ymin=0, ymax=yn, alpha=0.07, color=H_RED)
...
for lbl, lx, ly in [('STAR', (BCG_SHARE_THRESH+x1)/2, (BCG_GROWTH_THRESH+y1)/2),
    ...
    ax.text(lx, ly, lbl, ha='center', va='center', fontsize=8.5, color='#CCCCCC', fontweight='bold', alpha=0.9)

_bubble(ax, 'share', 'growth', 'vol_kl', max_vol)
ax.set_xlim(x0, x1); ax.set_ylim(y0, y1)
ax.set_xlabel('Portfolio Share — % of Total PSO Fuels Volume', fontsize=9, labelpad=5)
ax.set_ylabel('Volume Growth vs SPLY (%)', fontsize=9, labelpad=5)
ax.set_title('BCG Growth-Share Matrix (Internal Portfolio)', fontsize=10, fontweight='bold', color=H_NAVY, pad=9)
ax.annotate(f'Share threshold {BCG_SHARE_THRESH:.1f}%', xy=(BCG_SHARE_THRESH, y0+0.3), fontsize=7, color='#888', ha='center')
ax.annotate(f'Growth threshold {BCG_GROWTH_THRESH:.1f}%', xy=(x0+1, BCG_GROWTH_THRESH+0.15), fontsize=7, color='#888')
for vl, lbl in [(max_vol, f'{max_vol:,.0f} KL'), (max_vol*0.3, f'{max_vol*0.3:,.0f} KL'), (max_vol*0.07, f'{max_vol*0.07:,.0f} KL')]:
...
for lbl, lx, ly in [('CHAMPION', (vol_med+vx1)/2, (mgn_med+vy1)/2),
    ...
    ax2.text(lx, ly, lbl, ha='center', va='center', fontsize=8.5, color='#CCCCCC', fontweight='bold', alpha=0.9)
...
for vl, lbl in [(max_stns, f'{max_stns:,.0f} stns'), (max_stns*0.5, f'{max_stns*0.5:,.0f} stns'), (max_stns*0.15, f'{max_stns*0.15:,.0f} stns')]:
...
for lbl, lx, ly in [('OPTIMISE', (pen_med+dx1)/2, (prd_med+dy1)/2),
    ...
    ax3.text(lx, ly, lbl, ha='center', va='center', fontsize=8.5, color='#CCCCCC', fontweight='bold', alpha=0.9)

_bubble(ax3, 'stn_pct', 'kl_per_stn', 'vol_kl', max_vol)
ax3.set_xlim(dx0, dx1); ax3.set_ylim(dy0, dy1)
ax3.set_xlabel('Station Penetration (% of PSO Fuels Stations Stocking Category)', fontsize=9, labelpad=5)
...
bars = axL.barh(list(ypos), cat_s['growth'],
                 color=[PLC_HEX.get(cat_s.loc[c, 'plc'], H_GREY) for c in cat_s.index], alpha=0.85, zorder=3)
...
for bar, (c_nm, row) in zip(bars, cat_s.iterrows()):
    axL.annotate(f" {row.growth:+.1f}% — {row.plc}", xy=(bar.get_width(), bar.get_y()+bar.get_height()/2),
...
axR.bar(xp, cat_s['share'], color=[CAT_COLORS.get(c, H_GREY) for c in pareto.index], alpha=0.82, zorder=3)
axR2 = axR.twinx()
axR2.plot(xp, pareto['vol_cum'], 'o-', color=H_NAVY, lw=2, markersize=7, markerfacecolor='white', markeredgewidth=2, label='Cum. Vol %')
axR2.plot(xp, pareto['mgn_cum'], 's--', color=H_ORANGE, lw=1.5, markersize=6, markerfacecolor='white', markeredgewidth=2, label='Cum. Mgn %')
axR.set_xticks(xp)
...
Task: Per-category strategic signal reconciliation for PSO Fuels portfolio (Diesel, Petrol, Other Fuels, LPG).

Where framework signals conflict for a category, resolve the contradiction into a single coherent strategic directive.

Portfolio context:
- Total volume: {cat['vol_kl'].sum():,.0f} KL | BCG share threshold: ≥{BCG_SHARE_THRESH:.1f}% | Growth threshold: ≥{BCG_GROWTH_THRESH:.1f}%
...
{cat_block}
For EACH category provide exactly:
a) SIGNALS: Agree or conflict? Nature of tension if any.
b) DIRECTIVE (1 sentence): Single clear instruction.
...
def _prompt_portfolio(per_cat_text):
    top2 = cat['share'].sort_values(ascending=False).head(2)
    top2_share = top2.sum()
    top2_mgn = cat.loc[top2.index, 'mgn_cy'].sum() / cat['mgn_cy'].sum() * 100
    blended_pl = cat['mgn_cy'].sum() / cat['vol_kl'].sum() / 1000 if cat['vol_kl'].sum() else 0
    return f"""Period: {PERIOD}
Task: Portfolio-level strategy synthesis for PSO Fuels (Diesel, Petrol, Other Fuels, LPG).

## Per-Category Analysis (derived in prior step)
{per_cat_text}

## Portfolio Metrics
- {len(cat)} categories | Total: {cat['vol_kl'].sum():,.0f} KL
- {' + '.join(top2.index)} = {top2_share:.0f}% of volume and {top2_mgn:.0f}% of margin pool
- Growth range across categories: {cat['growth'].min():+.1f}% to {cat['growth'].max():+.1f}%
- Blended portfolio margin: PKR {blended_pl:.0f}/L
...
2. DOMINANT RISK (1 sentence): The single biggest strategic risk specific to this PSO fuels portfolio.
...
5. CONTRARIAN OBSERVATION (1-2 sentences): Something the frameworks reveal that contradicts conventional retail fuels portfolio wisdom."""
...
_txt(sl1, 1.0, 0.55, 8.0, 0.40, "PSO RETAIL FUELS", size=12, bold=True, color=RGBColor(0xBB, 0xCC, 0xFF), align=PP_ALIGN.CENTER)
_txt(sl1, 1.0, 1.00, 8.0, 0.75, "Strategic Framework Analysis", size=30, bold=True, align=PP_ALIGN.CENTER)
...
_txt(sl1, 1.0, 2.18, 8.0, 0.24, PERIOD, size=10, bold=True, color=C_ORANGE, align=PP_ALIGN.CENTER)
...
col_w = min(2.35, 9.0 / max(len(cols), 1))
for i, (title, items) in enumerate(cols):
    bx = 0.35 + i * (col_w + 0.10)
    _rect(sl1, bx, 2.55, col_w - 0.10, 2.78, RGBColor(0x10, 0x22, 0x44))
    _txt(sl1, bx+0.08, 2.62, col_w - 0.26, 0.22, title, size=8, bold=True, color=C_ORANGE)
    for j, (cat_name, pos) in enumerate(items):
        _txt(sl1, bx+0.08, 2.90 + j*0.60, col_w - 0.26, 0.22, cat_name, size=7.5, bold=True, color=RGBColor(0xBB, 0xCC, 0xFF))
        _txt(sl1, bx+0.08, 3.10 + j*0.60, col_w - 0.26, 0.20, pos, size=7, color=RGBColor(0xDD, 0xDD, 0xDD))

_txt(sl1, 0, 5.40, 10, 0.18, f"Hardcoded framework logic  |  AI: {provider or 'N/A'}  |  {PERIOD}",
     size=6.5, color=RGBColor(0x77, 0x88, 0xAA), align=PP_ALIGN.CENTER)

# ── Slide 2: BCG ──────────────────────────────────────────────────────────────
sl2 = _blank(prs)
_hdr(sl2, "BCG Growth-Share Matrix",
     f"Portfolio share threshold {BCG_SHARE_THRESH:.1f}%  |  Growth threshold {BCG_GROWTH_THRESH:.1f}%  |  Bubble = Volume CY  |  Applied to PSO internal portfolio")
_embed(sl2, fig_bcg, 0.18, 0.68, 6.10, 4.72)

rx = 6.40
_txt(sl2, rx, 0.68, 3.40, 0.20, "QUADRANT POSITIONS", size=7.5, bold=True, color=C_DNAVY)
for i, c_name in enumerate(MAIN):
    row = cat.loc[c_name]
    rgb = BCG_RGB.get(row.bcg, C_LGREY)
    _sidebar_card(sl2, rx, 0.92 + i*1.15, c_name, row.bcg,
                  f"Share {row.share:.1f}%  Growth {row.growth:+.1f}%  Vol {row.vol_kl:,.0f} KL", rgb)

# ── Slide 3: Volume-Margin ────────────────────────────────────────────────────
sl3 = _blank(prs)
_hdr(sl3, "Volume-Margin Matrix", f"Median split: Vol {vol_med:,.0f} KL  |  Margin PKR {mgn_med:,.0f}/L  |  Bubble = Stations")
_embed(sl3, fig_vm, 0.18, 0.68, 6.10, 4.72)

rx = 6.40
_txt(sl3, rx, 0.68, 3.40, 0.20, "QUADRANT POSITIONS", size=7.5, bold=True, color=C_DNAVY)
for i, c_name in enumerate(MAIN):
    row = cat.loc[c_name]
    rgb = VM_RGB.get(row.vm, C_LGREY)
    _sidebar_card(sl3, rx, 0.92 + i*1.15, c_name, row.vm,
                  f"Vol {row.vol_kl:,.0f} KL  Mgn PKR {row.mgn_pl:.0f}/L  Stns {int(row.stns):,}", rgb)

# ── Slide 4: Distribution Efficiency ─────────────────────────────────────────
sl4 = _blank(prs)
_hdr(sl4, "Distribution Efficiency Matrix", f"Median split: Penetration {pen_med:.0f}%  |  Productivity {prd_med:.2f} KL/stn  |  Bubble = Vol CY")
_embed(sl4, fig_de, 0.18, 0.68, 6.10, 4.72)

rx = 6.40
_txt(sl4, rx, 0.68, 3.40, 0.20, "QUADRANT POSITIONS", size=7.5, bold=True, color=C_DNAVY)
for i, c_name in enumerate(MAIN):
    row = cat.loc[c_name]
    rgb = DE_RGB.get(row.de, C_LGREY)
    _sidebar_card(sl4, rx, 0.92 + i*1.15, c_name, row.de,
...
# ── Save ──────────────────────────────────────────────────────────────────────
out = out_path('PSO_Fuels_Framework_Analysis', 'pptx', df)
prs.save(out)
print(f"\nSaved → {out}")
