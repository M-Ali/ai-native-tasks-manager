"""
PSO Fuels — Top 20 Cities Volume Profile (Word Document)
Mirrors workspace/lubes_vol_table.py, adapted to Fuels (category = FuelSegment).
...
CATS_PRESENT = [c for c in CAT_ORDER if c in fuels_all['FuelSegment'].unique()]

top20_cities = (fuels_all.groupby('CityNorm')['SalesLtr_CY'].sum()
...
for city in top20_cities:
    ...
    region = (cl.groupby('Sales office Region')['SalesLtr_CY'].sum().idxmax() if len(cl) else '')
    ...
    rows.append(dict(
        ...
        avg_stn=avg_stn, med_stn=med_stn, top10_pct=top10_pct,
        n_high=n_high, n_mid=n_mid, n_low=n_low, cat=cat_data,
    ))
...
r = p.add_run('PSO Retail Fuels — Top 20 Cities Volume Profile')
...
r2 = p2.add_run(f'Volume Edition  |  {REPORT_DATE}  |  Ranked by Total Fuels Volume CY')
...
sub1 = ['', '', '', '', 'Litres/1000', 'Litres/1000', 'vs SPLY', 'KL', 'KL',
        'count', 'count', '% of Vol', 'stns', 'stns', 'stns']
...
for ri, d in enumerate(rows, 2):
    ...
    vals = [str(ri-1), d['city'], d['region'], str(d['n_stns']),
            fmt_kl(d['vol_cy']), fmt_kl(d['vol_sply']), chg_str(chg),
            fmt_kl(d['avg_stn']), fmt_kl(d['med_stn']),
            str(d['n_active']), str(d['n_zero']), f"{d['top10_pct']:.1f}%",
            str(d['n_high']), str(d['n_mid']), str(d['n_low'])]
...
p4.paragraph_format.space_after = Pt(3); p4.paragraph_format.space_before = Pt(6)
...
for ri, d in enumerate(rows, 2):
    bg = 'F4F8FF' if ri % 2 == 0 else 'FFFFFF'
    chg = d['vol_chg']
    base_vals = [str(ri-1), d['city'], fmt_kl(d['vol_cy']), chg_str(chg)]
    cat_vals = []
    for cat in CAT_ORDER:
        cs = d['cat'][cat]
        c_chg = cs['vol_chg']
...
out = out_path('PSO_Fuels_Vol_Top20_Cities_Table', 'docx', df)
doc.save(out)
print(f"\nSaved: {out}")
