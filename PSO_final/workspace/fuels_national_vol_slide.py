"""
PSO National Fuels Volume Station Profile — Single Slide
Mirrors workspace/national_vol_slide.py (Lubricants), adapted to Fuels.
Aggregates all retail fuels stations nationally (not just top cities).
Output: reports/PSO_National_Fuels_Vol_Station_Profile_<period>.pptx
"""
import sys, os
...
fuels_vol      = fuels_all['SalesLtr_CY'].sum()
fuels_vol_sply = fuels_all['SalesLtr_SPLY'].sum()
vol_chg        = (fuels_vol - fuels_vol_sply) / fuels_vol_sply * 100 if fuels_vol_sply else 0

stn_total = (fuels_all.groupby('Customer Number')['SalesLtr_CY']
...
for code in top10_codes:
    stn_data = fuels_all[fuels_all['Customer Number'] == code]
    ...
    top10_detail.append({'code': code, 'name': name, 'city': city, 'total': total, 'by_cat': by_cat})

cat_stats = {}
for cat in CAT_ORDER:
    df_cat = fuels_all[fuels_all['FuelSegment'] == cat]
    ...
    cat_stats[cat] = dict(
        vol=vol,
        vol_pct=vol / fuels_vol * 100 if fuels_vol else 0,
...
d = dict(
    ...
    fuels_vol=fuels_vol, vol_chg=vol_chg,
...
def build_national_slide(prs, d):
    ...
    txt(sl, 0.25, 0.10, 6.5, 0.20, "PSO RETAIL — NATIONAL FUELS", size=8, color=C['TEAL_L'])
    txt(sl, 0.25, 0.27, 6.5, 0.26, "Station Volume Performance — Who Is Delivering",
        size=17, bold=True, color=C['WHITE'])
    txt(sl, 6.5, 0.20, 3.25, 0.18,
        f"Tiers based on Total Fuels Volume  |  Median {fmt_kl(d['med_per_stn'])}  |  {REPORT_PERIOD}",
        size=8, color=C['ORANGE_L'], align=PP_ALIGN.RIGHT)

    kpis = [
        (fmt_kl_total(d['fuels_vol']), "Total Volume", C['ORANGE'], chg_str(d['vol_chg'])),
    ...
    for ki, (val, lbl, col, sub) in enumerate(kpis):
        ...
        txt(sl, kx+0.06, 0.71, kpi_w-0.08, 0.24, val, size=13, bold=True, color=col)
        txt(sl, kx+0.06, 0.94, kpi_w-0.08, 0.13, lbl, size=6.5, color=C['GREY'])
        txt(sl, kx+0.06, 1.09, kpi_w-0.08, 0.11, sub, size=6, color=C['GREY2'])

    txt(sl, 0.19, 1.32, 3.26, 0.14, "STATION VOLUME TIERS", size=7.5, bold=True, color=C['GREY'])

    tiers = [
        (C['GREEN'], f"HIGH PERFORMERS    {d['n_high']} stations", fmt_kl_total(d['vol_high']),
         f"{d['vol_high']/d['fuels_vol']*100:.1f}% of National Fuels  |  Above {fmt_kl(d['p75'])} each" if d['fuels_vol'] else "N/A"),
        (C['BLUE'], f"MID PERFORMERS    {d['n_mid']} stations", fmt_kl_total(d['vol_mid']),
         f"{d['vol_mid']/d['fuels_vol']*100:.1f}% of National Fuels  |  {fmt_kl(d['med_per_stn'])} – {fmt_kl(d['p75'])}" if d['fuels_vol'] else "N/A"),
        (C['AMBER'], f"LOW PERFORMERS    {d['n_low']} stations", fmt_kl_total(d['vol_low']),
         f"{d['vol_low']/d['fuels_vol']*100:.1f}% of National Fuels  |  Below {fmt_kl(d['med_per_stn'])}" if d['fuels_vol'] else "N/A"),
        (C['RED'], f"ZERO FUELS    {d['n_zero_stn']} stations", "0 KL", "Not selling any fuel product"),
    ]
    ...
    txt(sl, 3.56, 0.68, 6.35, 0.14, "TOP 10 STATIONS NATIONALLY BY TOTAL FUELS VOLUME (KL)",
    ...
    for si, stn in enumerate(top10[:10]):
        ...
        txt(sl, chart_x, ry+0.02, 1.90, row_h-0.04, label[:28], size=7, color=C['DNAV2'])
    ...
    ins_txt = (
        f"Top 10 stations = {fmt_kl_total(d['top10_vol'])} ({d['top10_pct']:.1f}% of national fuels volume). "
        f"{top_name} ({top_city}) leads at {fmt_kl(top10[0]['total']) if top10 else 'N/A'}. "
        f"{d['n_zero_stn']} stations across the network sell zero fuels — "
...
out = out_path('PSO_National_Fuels_Vol_Station_Profile', 'pptx', df)
prs.save(out)
print(f"Saved → {out}")
