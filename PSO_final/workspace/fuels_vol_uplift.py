"""
PSO Fuels — Volume Uplift Potential from 'Where to Focus' initiatives.
Mirrors workspace/lubes_vol_uplift.py, adapted to Fuels:
  I1 = Activate stations not selling R95 (premium petrol upsell)
  I2 = Activate stations not selling Diesel/HSD (cross-sell to petrol-only stations)
  I3 = Upgrade low-tier stations toward city median total-fuels volume
  I4 = Convert zero-fuels stations (active retail stations selling no fuel at all)
Produces:
  1. Word table (top 20 cities x 4 initiatives x conservative & optimal)
...
def ct(cell, text, bold=False, size=8, color=None, align=WD_ALIGN_PARAGRAPH.CENTER, italic=False):
    cell.text = ''
...
fuels_all = retail[retail['FuelSegment'].isin(['Diesel', 'Petrol', 'Other Fuels', 'LPG'])].copy()

top20 = (fuels_all.groupby('CityNorm')['SalesLtr_CY'].sum()
         .sort_values(ascending=False).head(20).index.tolist())

rows = []
for city in top20:
    cl = fuels_all[fuels_all['CityNorm'] == city]
    ...
    # I1: R95 gap (petrol-selling stations not carrying R95)
    petrol = cl[cl['FuelSegment'] == 'Petrol']
    ...
    r95_n_sell = (r95_sv > 0).sum()
    r95_avg = r95_sv[r95_sv > 0].mean() if r95_n_sell else 0
    r95_gap = max(0, n_petrol_stns - r95_n_sell)

    # I2: Diesel/HSD gap (retail stations not carrying HSD)
    hsd = cl[cl['ProductCategory'] == 'HSD']
    hsd_sv = hsd.groupby('Customer Number')['SalesLtr_CY'].sum()
    hsd_n_sell = (hsd_sv > 0).sum()
    hsd_avg = hsd_sv[hsd_sv > 0].mean() if hsd_n_sell else 0
    hsd_gap = max(0, stn_all - hsd_n_sell)

    fp1 = r95_gap * r95_avg
    fp2 = hsd_gap * hsd_avg
    fp3 = max(0, (med_stn - avg_low)) * n_low
    fp4 = n_zero * avg_stn

    rows.append(dict(
        city=city, vol_cy=vol_cy, vol_chg=vol_chg_pct,
        n_stns=stn_all, n_zero=n_zero, n_low=n_low,
        r95_gap=r95_gap, hsd_gap=hsd_gap,
        fp1=fp1, fp2=fp2, fp3=fp3, fp4=fp4, fp_total=fp1 + fp2 + fp3 + fp4,
        c1=fp1 * CONSERV, c2=fp2 * CONSERV, c3=fp3 * CONSERV, c4=fp4 * CONSERV,
        o1=fp1 * OPTIMAL, o2=fp2 * OPTIMAL, o3=fp3 * OPTIMAL, o4=fp4 * OPTIMAL,
        c_total=(fp1 + fp2 + fp3 + fp4) * CONSERV, o_total=(fp1 + fp2 + fp3 + fp4) * OPTIMAL,
...
r = p.add_run('PSO Fuels — Volume Uplift Potential from Sales Initiatives (Top 20 Cities)')
...
GRP = {
    ...
    4: '0B5E32', 5: '0B5E32', 6: '0B5E32', 7: '0B5E32',
    8: '5B2C6F', 9: '5B2C6F', 10: '5B2C6F', 11: '5B2C6F',
    12: '784212', 13: '784212', 14: '784212', 15: '784212',
    16: '8B0000', 17: '8B0000', 18: '8B0000', 19: '8B0000',
    20: '1A3A6E', 21: '145A32', 22: '7D6608',
    23: '145A32', 24: '7D6608',
}
...
for ri, d in enumerate(rows, 2):
    ...
    vals = [
        ...
        str(d['r95_gap']), fmt(d['fp1']), fmt(d['c1']), fmt(d['o1']),
        str(d['hsd_gap']), fmt(d['fp2']), fmt(d['c2']), fmt(d['o2']),
        str(d['n_low']),   fmt(d['fp3']), fmt(d['c3']), fmt(d['o3']),
        str(d['n_zero']),  fmt(d['fp4']), fmt(d['c4']), fmt(d['o4']),
        fmt(d['fp_total']), fmt(d['c_total']), fmt(d['o_total']),
        f"+{d['c_pct']:.1f}%", f"+{d['o_pct']:.1f}%",
    ]
...
ct(tbl.cell(TR, 1), 'NATIONAL TOTAL', bold=True, size=7.5, color=W_WHITE, align=WD_ALIGN_PARAGRAPH.LEFT)
...
tot_vals = [
    fmt(sum(r['r95_gap'] for r in rows)), fmt(sum(r['fp1'] for r in rows)), fmt(nat_c1), fmt(nat_o1),
    fmt(sum(r['hsd_gap'] for r in rows)), fmt(sum(r['fp2'] for r in rows)), fmt(nat_c2), fmt(nat_o2),
    ...
    f"+{nat_c_tot/nat_vol*100:.1f}%" if nat_vol else "—", f"+{nat_o_tot/nat_vol*100:.1f}%" if nat_vol else "—",
]
...
notes = [
    ...
    ('I1 = Activate petrol-selling stations not carrying R95 (premium upsell).  '
     'I2 = Activate stations not carrying Diesel/HSD (cross-sell).  '
     'I3 = Upgrade low-volume stations (below median) toward city median.  '
     'I4 = Convert zero-fuels stations (active retail stations selling no fuel).  '
...
out_word = out_path('PSO_Fuels_Vol_Uplift_Table', 'docx', df)
doc.save(out_word)
...
PC = {'NAVY': '0F2035', 'DNAVY': '0A1628', 'TEAL': '148F77', 'TEAL_L': 'A2D9CE',
      'GOLD': 'D4AC0D', 'WHITE': 'FFFFFF', 'DARK': '1B2A4A', 'ORANGE_L': 'E59866'}
...
def ptxt(sl, x, y, w, h, text, size=9, bold=False, color='FFFFFF', align=PP_ALIGN.LEFT, wrap=True):
...
ax2.set_title('Projected Total Volume — Top 20 Cities', fontsize=9, fontweight='bold', color='#0A1628', pad=6)
...
for bar, val, lbl in zip(bars, values,
    [f'{nat_vol/1000:.0f} KL\n(Base)',
     f'{(nat_vol+nat_c_tot)/1000:.0f} KL\n(+{nat_c_tot/nat_vol*100:.1f}%)' if nat_vol else 'N/A',
     f'{(nat_vol+nat_o_tot)/1000:.0f} KL\n(+{nat_o_tot/nat_vol*100:.1f}%)' if nat_vol else 'N/A']):
...
ptxt(sl, 0.25, 0.13, 7.5, 0.32, 'Fuels Volume Uplift Potential — Where to Focus Initiatives',
     size=17, bold=True, color=PC['WHITE'])
ptxt(sl, 7.5, 0.18, 2.30, 0.20, f'Top 20 Cities  |  {REPORT_DATE}',
     size=8.5, color=PC['ORANGE_L'], align=PP_ALIGN.RIGHT)

sl.shapes.add_picture(chart_buf, PInches(0.18), PInches(0.66), PInches(5.70), PInches(3.10))

cx = 6.02; cy = 0.66; cw = 1.84; ch = 4.68
prect(sl, cx, cy, cw, ch, PC['TEAL'])
ptxt(sl, cx + 0.12, cy + 0.10, cw - 0.20, 0.24, 'CONSERVATIVE', size=9, bold=True, color=PC['WHITE'])
ptxt(sl, cx + 0.12, cy + 0.36, cw - 0.20, 0.14, '25% of full potential', size=7, color=PC['TEAL_L'])
ptxt(sl, cx + 0.08, cy + 0.56, cw - 0.14, 0.45, fmt(nat_c_tot) + ' KL', size=20, bold=True, color=PC['WHITE'])
ptxt(sl, cx + 0.08, cy + 1.04, cw - 0.14, 0.18,
     f"+{nat_c_tot/nat_vol*100:.1f}% uplift on current" if nat_vol else "N/A",
     size=8.5, bold=True, color=PC['TEAL_L'])
ptxt(sl, cx + 0.08, cy + 1.30, cw - 0.14, 0.14, f"New total:  {fmt(nat_vol+nat_c_tot)} KL", size=8, color=PC['WHITE'])
prect(sl, cx + 0.08, cy + 1.58, cw - 0.16, 0.02, PC['TEAL_L'])
lines_c = [('I1 R95', fmt(nat_c1)), ('I2 HSD', fmt(nat_c2)), ('I3 Low-Tier', fmt(nat_c3)), ('I4 Zero Stns', fmt(nat_c4))]
...
ptxt(sl, ox + 0.12, oy + 0.10, cw - 0.20, 0.24, 'OPTIMAL', size=9, bold=True, color=PC['DARK'])
ptxt(sl, ox + 0.12, oy + 0.36, cw - 0.20, 0.14, '55% of full potential', size=7, color=PC['DARK'])
ptxt(sl, ox + 0.08, oy + 0.56, cw - 0.14, 0.45, fmt(nat_o_tot) + ' KL', size=20, bold=True, color=PC['DARK'])
ptxt(sl, ox + 0.08, oy + 1.04, cw - 0.14, 0.18,
     f"+{nat_o_tot/nat_vol*100:.1f}% uplift on current" if nat_vol else "N/A",
     size=8.5, bold=True, color=PC['DARK'])
ptxt(sl, ox + 0.08, oy + 1.30, cw - 0.14, 0.14, f"New total:  {fmt(nat_vol+nat_o_tot)} KL", size=8, color=PC['DARK'])
prect(sl, ox + 0.08, oy + 1.58, cw - 0.16, 0.02, PC['DARK'])
lines_o = [('I1 R95', fmt(nat_o1)), ('I2 HSD', fmt(nat_o2)), ('I3 Low-Tier', fmt(nat_o3)), ('I4 Zero Stns', fmt(nat_o4))]
...
out_pptx = out_path('PSO_Fuels_Uplift_Scenarios', 'pptx', df)
prs.save(out_pptx)
print(f"PPTX saved: {out_pptx}")

print(f"\n{'─'*55}")
print(f"  Current volume (top 20):  {nat_vol/1000:>8.1f} KL")
print(f"  Full potential:           {nat_fp_tot/1000:>8.1f} KL  (+{nat_fp_tot/nat_vol*100:.1f}%)" if nat_vol else "  Full potential: N/A")
print(f"  Conservative uplift:      {nat_c_tot/1000:>8.1f} KL  (+{nat_c_tot/nat_vol*100:.1f}%)" if nat_vol else "  Conservative uplift: N/A")
print(f"  Optimal uplift:           {nat_o_tot/1000:>8.1f} KL  (+{nat_o_tot/nat_vol*100:.1f}%)" if nat_vol else "  Optimal uplift: N/A")
print(f"{'─'*55}")
