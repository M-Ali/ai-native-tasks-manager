"""
PSO City Fuels Profile Generator — VOLUME EDITION
Mirrors workspace/city_profiles_volume.py (Lubricants), adapted to Fuels.
Same 5-slide format as fuels_city_profiles.py but metric = SalesLtr_CY (litres/KL).
Saves to <PSO_OUTDIR>/fuels_city_profiles_volume/ — does NOT overwrite revenue profiles.
"""
import sys, os
...
def txt(slide, x, y, w, h, text, size=9, bold=False, color='FFFFFF', align=PP_ALIGN.LEFT, wrap=True):
...
def compute_city(city_norm):
    ...
    fuels_vol = city_fuels['SalesLtr_CY'].sum()
    fuels_vol_sply = city_fuels['SalesLtr_SPLY'].sum()
    vol_chg = (fuels_vol - fuels_vol_sply) / fuels_vol_sply * 100 if fuels_vol_sply else 0

    stn_total = (city_fuels.groupby('Customer Number')['SalesLtr_CY'].sum().sort_values(ascending=False))
    n_stns = city_all['Customer Number'].nunique()
    n_active = (stn_total > 0).sum()
    n_zero = n_stns - n_active

    avg_per_stn = fuels_vol / n_stns if n_stns else 0
    ...
    top10_pct = top10_vol / fuels_vol * 100 if fuels_vol else 0

    active_cats = [c for c in CAT_ORDER if city_fuels[city_fuels['FuelSegment'] == c]['SalesLtr_CY'].sum() > 0]
    ...
    for cat in CAT_ORDER:
        ...
        cat_stats[cat] = dict(
            vol=vol, vol_pct=vol/fuels_vol*100 if fuels_vol else 0,
    ...
    for code in top10_codes:
        ...
        by_cat = {cat: stn_data[stn_data['FuelSegment'] == cat]['SalesLtr_CY'].sum() for cat in CAT_ORDER}
        top10_detail.append({'code': code, 'name': name, 'total': total, 'by_cat': by_cat})

    petrol = city_fuels[city_fuels['FuelSegment'] == 'Petrol']
    r95_sv = petrol[petrol['ProductCategory'] == 'R95'].groupby('Customer Number')['SalesLtr_CY'].sum()
    pmg_sv = petrol[petrol['ProductCategory'] == 'PMG'].groupby('Customer Number')['SalesLtr_CY'].sum()
    ...
    hsd_sv = hsd.groupby('Customer Number')['SalesLtr_CY'].sum()
    ...
    return dict(
        ...
        fuels_vol=fuels_vol, fuels_vol_sply=fuels_vol_sply, vol_chg=vol_chg,
...
def build_slide1(prs, d):
    ...
    txt(sl, 1.54, 1.47, 6.0, 0.20, f"PSO {city.upper()} — RETAIL FUELS DIVISION", size=9, color=C['TEAL_L'])
    rect(sl, 0.93, 1.81, 0.035, 0.90, C['TEAL'])
    txt(sl, 1.27, 1.75, 7.98, 0.60, f"{city} Station Profile", size=34, bold=True, color=C['WHITE'])
    txt(sl, 1.27, 2.44, 7.98, 0.28, "Fuels Volume Analysis", size=14, color=C['ORANGE_L'])
    rect(sl, 1.27, 2.80, 1.60, 0.22, C['TEAL'])
    txt(sl, 1.30, 2.82, 1.54, 0.18, "VOLUME EDITION", size=8, bold=True, color=C['WHITE'])
    ...
    kpis = [
        ...
        (2.20, 2.05, fmt_kl_total(d['fuels_vol']), "Total Fuels Volume"),
    ...
    for bx, bw, val, lbl in kpis:
        ...
        txt(sl, bx+0.18, 3.86, bw-0.22, 0.16, lbl, size=8, color=C['TEAL_L'])

# ── SLIDE 2: At a Glance ─────────────────────────────────────────────────────
def build_slide2(prs, d):
    sl = prs.slides.add_slide(prs.slide_layouts[6])
    rect(sl, 0, 0, 10, 0.61, C['DNAVY'])
    txt(sl, 0.25, 0.14, 6.5, 0.32, f"{d['city']} — Fuels Volume At a Glance", size=17, bold=True, color=C['WHITE'])
    txt(sl, 6.5, 0.20, 3.25, 0.20,
        f"{d['n_stns']} Stations  |  {fmt_kl_total(d['fuels_vol'])} Total Volume  |  {REPORT_PERIOD}",
        size=8.5, color=C['ORANGE_L'], align=PP_ALIGN.RIGHT)

    lx, lw = 0.22, 3.06
    vol_chg_str = chg_str(d['vol_chg'])
    boxes = [
        (0.75, 0.86, fmt_kl_total(d['fuels_vol']), 'TOTAL FUELS VOLUME', C['ORANGE'],
         f"vs SPLY: {vol_chg_str}  (SPLY: {fmt_kl_total(d['fuels_vol_sply'])})"),
        (1.73, 0.86, fmt_kl(d['avg_per_stn']), 'AVG VOLUME PER STATION', C['BLUE'],
    ...
    rx = 3.47
    txt(sl, rx, 0.73, 6.5, 0.15, "VOLUME CONTRIBUTION BY CATEGORY", size=8, bold=True, color=C['GREY'])
    active = [c for c in CAT_ORDER if d['cat'][c]['vol'] > 0]
    cat_max_vol = max((d['cat'][c]['vol'] for c in active), default=1) or 1
    track_w = 3.90
    ...
    for i, cat in enumerate(active):
        ...
        txt(sl, rx+1.10+track_w+0.06, by+0.03, 1.25, 0.14,
            f"{cs['vol_pct']:.1f}%  |  {cs['n_selling']} stns  {chg_str(cs['vol_chg'])}", size=7, color=C['DNAV2'])
    ...
    top2 = sorted(active, key=lambda c: d['cat'][c]['vol'], reverse=True)[:2]
    top2_pct = sum(d['cat'][c]['vol_pct'] for c in top2)
    widest = max(active, key=lambda c: d['cat'][c]['n_selling']) if active else 'Diesel'
    fastest = max(active, key=lambda c: d['cat'][c]['vol_chg']) if active else 'Diesel'
    insight = (
        f"{' + '.join(top2)} = {top2_pct:.0f}% of {d['city']} fuels volume. "
        f"{widest} has the widest reach ({d['cat'][widest]['n_pct']:.0f}% of stations). "
        f"{fastest} is the fastest growing category ({chg_str(d['cat'][fastest]['vol_chg'])} vs SPLY). "
        f"{d['no_r95']} petrol stations don't carry R95 — premium upsell opportunity."
    )
...
def build_slide3(prs, d):
    ...
    txt(sl, 0.25, 0.14, 7.2, 0.30, "Category Volume Performance — Avg, Min & Max per Station",
        size=17, bold=True, color=C['WHITE'])
    txt(sl, 7.25, 0.20, 2.55, 0.18, f"{d['n_stns']} Stations Total  |  All values in KL",
        size=9, color=C['ORANGE_L'], align=PP_ALIGN.RIGHT)

    active_cats = [c for c in CAT_ORDER if d['cat'][c]['vol'] > 0 and d['cat'][c]['n_selling'] > 0]
    ...
    for i, cat in enumerate(active_cats):
        ...
        txt(sl, vx, ry, 2.04, rh*0.50, f"Avg  {fmt_kl(cs['avg'])}", size=8, bold=True, color=col)
        txt(sl, vx, ry+rh*0.45, 2.04, rh*0.55, f"Med {fmt_kl(cs['median'])}  Max {fmt_kl(cs['max_val'])}", size=7, color=C['GREY2'])

    by_vol = sorted(active_cats, key=lambda c: d['cat'][c]['vol'], reverse=True)
    by_cov = sorted(active_cats, key=lambda c: d['cat'][c]['n_pct'])

    insight_data = []
    if by_vol:
        lead = by_vol[0]; cs = d['cat'][lead]
        not_sell = d['n_stns'] - cs['n_selling']
        insight_data.append((
            C['RED_BG'], C['RED_D'], f"{lead.upper()} — VOLUME LEADER, COVERAGE GAP",
            f"{lead} drives {cs['vol_pct']:.0f}% of city volume ({chg_str(cs['vol_chg'])} vs SPLY). "
            f"Yet {not_sell} stations are not stocking it. Avg volume per active station: {fmt_kl(cs['avg'])}. "
            f"Closing this gap could add ~{fmt_kl(not_sell * cs['avg'] * 0.5)} to city volume."
        ))
    if len(by_cov) > 1:
        reliable = by_cov[-1]; cs = d['cat'][reliable]
        insight_data.append((
            C['BG_GREEN'], C['BLUE2'], f"{reliable.upper()} — MOST CONSISTENT VOLUMES",
            f"{cs['n_pct']:.0f}% station coverage. Distribution is "
            f"{'tight' if cs['max_val'] < 5*cs['avg'] else 'moderate'} "
            f"(avg {fmt_kl(cs['avg'])}, median {fmt_kl(cs['median'])}). "
            f"Volume {'growing' if cs['vol_chg'] >= 0 else 'declining'} at {chg_str(cs['vol_chg'])} vs SPLY."
        ))
    if by_cov:
        gap_cat = by_cov[0]; cs = d['cat'][gap_cat]
        not_sell2 = d['n_stns'] - cs['n_selling']
        insight_data.append((
            C['BG_YELLOW'], C['BROWN'], f'{gap_cat.upper()} — {not_sell2} STATIONS NOT STOCKING',
            f"{gap_cat} has {cs['n_pct']:.0f}% station coverage — {not_sell2} stations are missing it. "
            f"Activating these at avg {fmt_kl(cs['avg'])} adds ~{fmt_kl(not_sell2 * cs['avg'] * 0.5)} volume."
        ))
...
def build_slide4(prs, d):
    sl = prs.slides.add_slide(prs.slide_layouts[6])
    rect(sl, 0, 0, 10, 0.58, C['DNAVY'])
    txt(sl, 0.25, 0.14, 6.5, 0.30, "Station Volume Performance — Who Is Delivering", size=17, bold=True, color=C['WHITE'])
    txt(sl, 6.5, 0.20, 3.25, 0.18, f"Tiers based on Total Fuels Volume  |  Median {fmt_kl(d['med_per_stn'])}",
        size=8.5, color=C['ORANGE_L'], align=PP_ALIGN.RIGHT)

    txt(sl, 0.19, 0.68, 3.26, 0.14, "STATION VOLUME TIERS", size=7.5, bold=True, color=C['GREY'])
    tiers = [
        (C['GREEN'], f"HIGH PERFORMERS    {d['n_high']} stations", fmt_kl_total(d['vol_high']),
         f"{d['vol_high']/d['fuels_vol']*100:.1f}% of {d['city']} Fuels  |  Above {fmt_kl(d['p75'])} each" if d['fuels_vol'] else "N/A"),
        (C['BLUE'], f"MID PERFORMERS    {d['n_mid']} stations", fmt_kl_total(d['vol_mid']),
         f"{d['vol_mid']/d['fuels_vol']*100:.1f}% of {d['city']} Fuels  |  {fmt_kl(d['med_per_stn'])} – {fmt_kl(d['p75'])}" if d['fuels_vol'] else "N/A"),
        (C['AMBER'], f"LOW PERFORMERS    {d['n_low']} stations", fmt_kl_total(d['vol_low']),
         f"{d['vol_low']/d['fuels_vol']*100:.1f}% of {d['city']} Fuels  |  Below {fmt_kl(d['med_per_stn'])}" if d['fuels_vol'] else "N/A"),
        (C['RED'], f"ZERO FUELS    {d['n_zero_stn']} stations", "0 KL", "Not selling any fuel category"),
    ]
    ...
    txt(sl, 3.56, 0.68, 6.35, 0.14, "TOP 10 STATIONS BY TOTAL FUELS VOLUME (KL)", size=7.5, bold=True, color=C['GREY'])
    top10 = d['top10']
    ...
    for si, stn in enumerate(top10[:10]):
        ...
        txt(sl, chart_x, ry+0.02, 1.75, row_h-0.04, str(stn['name'])[:22], size=7, color=C['DNAV2'])
        ...
        for cat in CAT_ORDER:
            ...
            if vol > 0:
                ...
                bx_start += seg_w
        txt(sl, bx_start+0.06, ry+0.02, 0.90, row_h-0.04, fmt_kl(stn['total']), size=7, color=C['GREY2'])
    ...
    ins_txt = (f"Top 10 stations = {fmt_kl_total(d['top10_vol'])} ({d['top10_pct']:.1f}% of all {d['city']} fuels volume). "
               f"{top10_names[0]} leads at {fmt_kl(top10[0]['total']) if top10 else 'N/A'} — "
...
def build_slide5(prs, d):
    ...
    txt(sl, 7.5, 0.20, 2.30, 0.18, "4 Priorities  |  Estimated Volume Upside", size=8.5, color=C['ORANGE_L'], align=PP_ALIGN.RIGHT)
    ...
    priorities = [
        (C['BG_LIGHT'], C['BLUE'], C['BLUE2'],
         ...
         f"R95 carries the highest margin per litre and is growing faster than PMG nationally. "
         f"{no_r95} petrol-selling stations in {d['city']} are not stocking it. "
         f"Avg R95 volume per active station: {fmt_kl(r95_avg)}.",
         fmt_kl(no_r95 * r95_avg * 0.5),
         "HIGH MARGIN  Immediate Action"),

        (C['BG_GREEN'], C['GREEN'], C['BLUE2'],
         '02', f"Activate {no_hsd} Stations Not Selling Diesel/HSD",
         f"Diesel is a core volume driver. {no_hsd} stations in {d['city']} are missing out. "
         f"Avg HSD volume per active station: {fmt_kl(hsd_avg)}.",
         fmt_kl(no_hsd * hsd_avg * 0.5),
        ...
        (C['RED_BG'], C['RED'], C['RED_D'],
         ...
         f"{n_zero} active retail stations are selling ZERO fuels. "
...
profiles_dir = os.path.join(os.environ.get('PSO_OUTDIR', 'reports'), 'fuels_city_profiles_volume')
...
for city in target_cities:
    ...
    out = out_path(f"PSO_{city.replace(' ', '_')}_Fuels_Vol_Profile", 'pptx', df, out_dir=profiles_dir)
    prs.save(out)
    print(f"saved → {out}")
